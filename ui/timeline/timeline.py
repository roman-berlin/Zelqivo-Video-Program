...
class ClipItem(QGraphicsRectItem):
    """Draggable timeline clip. Horizontal move only; snaps to grid on release.

    (Why) Keep reordering simple and readable; text is elided for readability.
    """
    ...
    def itemChange(self, change, value):  # type: ignore[override]
        if change == QGraphicsRectItem.GraphicsItemChange.ItemPositionChange:
            p: QPointF = value
            return QPointF(max(0.0, p.x()), 0.0)
        return super().itemChange(change, value)

    def mouseReleaseEvent(self, event):  # type: ignore[override]
        x = self.x()
        snapped = round(x / self._grid_px) * self._grid_px
...
class TimelineScene(QGraphicsScene):
    """Scene with grid and clips. Emits orderChanged when clips are rearranged."""
    ...
    def paths_order(self) -> List[str]:
        return [c.path for c in self._sorted_clips()]

    def relayout_compact(self) -> None:
        x = 0.0
        for c in self._sorted_clips():
            c.setPos(x, 0)
            x += c.rect().width() + 8
        self._emit_order_if_changed()

    def _sorted_clips(self) -> List[ClipItem]:
    ...
    def _emit_order_if_changed(self) -> None:
        new_order = [c.path for c in self._sorted_clips()]
        self.orderChanged.emit(new_order)


class TimelineView(QGraphicsView):
    """Graphics view with Ctrl+wheel zoom and Space+drag panning."""
